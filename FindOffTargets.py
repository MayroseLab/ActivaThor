...
def get_off_target(x, sequence_to_candidate_dict: Dict[str, ActivationCandidate]):
    ...
    for char in off_target.seq:
        if char not in {"A", "C", "T", "G"}:
            wrong_letters = False
            break
    on_target = True  # TODO change confusing name
    if off_target.number_of_mismatches == 0:
        on_target = False
    if wrong_letters and on_target:
...
def get_off_targets(candidates_list: List[ActivationCandidate], in_path: str, out_path: str):
    """
