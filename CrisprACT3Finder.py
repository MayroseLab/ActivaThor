...
def create_candidates(targets_lst: List[str], scores_lst: List[float], gene_id, chromosome, act_site_start,
                      act_site_end,
...
def return_candidates(sites_lst: List[str], genes_filt_df, scoring_function, pams: List[str], min_gc_cont: int,
    ...
    for i in range(0, len(sites_lst), 2):
        ...
        candidates_list += create_candidates(targets_fwd, scores_fwd, gene_id, chromosome, act_site_start, act_site_end,
                                             strand, "+", min_gc_cont, max_gc_cont)
        candidates_list += create_candidates(targets_rev, scores_rev, gene_id, chromosome, act_site_start, act_site_end,
                                             strand, "-", min_gc_cont, max_gc_cont)
    return candidates_list


def results_to_dataframe(candidates_list: List[ActivationCandidate]):
    df = pd.DataFrame([candidate.to_dict() for candidate in candidates_list])
    return df


def sgrnas_for_genes(out_path: str, in_path: str, genes_ids: List[str], fasta_file: str, gff_file: str,
    ...
    genes_filt_df = filter_gff_file(gff_file, genes_ids, bp_upstream, bp_downstream)  # TODO elaborate the actions taken
    ...
    get_off_targets(candidates_list, in_path, out_path)
    # sort
    candidates_list.sort(
        key=lambda c: (c.gene, c.gc_content_category, -sum(c.nucleotide_repetitions.values()), -c.on_score))
    # results to DataFrame
    df = results_to_dataframe(candidates_list)
    header = ['seq', "pam", "gene", "chromosome", "act_site_start", "act_site_end", "strand", "gc_content", "gc_content_category", "on_score", "off1", "off2"]
